...
Se observa que HIP.D, HIP.I, PRE.D y PRE.I apenas tienen variabilidad.
...
Se observa que todas las variables presentan una distribución no normal. A la vista de los histogramas, el problema es que existen muchos individuos sin defectos visuales (valores 0), lo que provoca que la distribución sea asimétrica y no normal.

### Defectos visuales (sin individuos sin defectos visuales)

```{r normalidad-dep-sin-cero}
df |>
  select(all_of(vars.dep)) |>
  pivot_longer(everything(), names_to = "Variable", values_to = "Valor") |>
  filter(Valor != 0) |>
...
Aún eliminado los ceros, las variables presentan distribución no normal.
...
Todas las variables excepto VHC.BT3 y VHL.BT2, tienen una evolución no normal.
